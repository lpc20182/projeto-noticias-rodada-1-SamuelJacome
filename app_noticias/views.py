...
def categoria_Economia_recentes(request):
    categoria = Categoria.objects.get(slug='economia')
    ultimo_destaque = Noticia.objects.filter(destaque=True).last()
    noticias = Noticia.objects.filter(categoria=categoria).exclude(id = ultimo_destaque.id).order_by('-data_de_publicacao')[:5]
    return render(request, 'app_noticias/recentes_economia.html', {
        'noticias': noticias,
        'categoria':categoria,
        })